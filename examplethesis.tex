...
1. Numbers (\verb|\b(?:\d+(?:\.\d\+)?\d+(?:\_\d\+)\+)\b|): Matches integer and floating-point literals, including those with underscores. For example:
\begin{minted}{rust}
let c = 3.14;
\end{minted}

2. Arrays (\verb|\[(\s*\d+\s*(?:,\s*\d+\s*)*)\]|): Matches array literals, excluding array declarations to avoid double matching. For example, this will be only one match:

\begin{minted}{rust}
let a : [i32; 5] = [1, 2, 3, 4, 5];  
\end{minted}

3. References (\verb|&[mut\s]*\w+|): Matches mutable and immutable reference types. It will match \texttt{\&T} and \texttt{\&mut T}.

\begin{minted}{rust}
let y = &mut x;  
\end{minted}

4. Heap Types (\verb|Box<[<>]+>|Rc<[<>]+>|Arc<[<>]+>|): Matches Box, Rc, and Arc heap-allocated type declarations. For example:

\begin{minted}{rust}
let boxed_value: Box<i32>
\end{minted}
5. Dynamic Vectors (\verb|vec!|Vec::new|): Matches dynamic vector types declaration in Rust and the vec![] macro.

\begin{minted}{rust}
let v = Vec::new();
\end{minted}
6. Strings (\verb|String::|.to\_string|format!|): Matches String creation and manipulation through various methods and macros.
...
1. Numbers: This pattern uses Ada keywords for numbers (\texttt{Integer, Float, Modular...}) and matches various numeric types such as integer, float, fixed, decimal, modular, natural, positive, long, and range. We choose to proceed differently and list all possible types as SPARK requires initialization.

\begin{minted}{ada}
type Float is digits 6;
\end{minted}

2. Structs (\verb|type\s\+\w\+\s\+is\s\+record|). This pattern matches record types, which are similar to structs in other languages.
...
3. Enums: (\verb|type\s\+\w\+\s\+is\s\+\(|). Matches enumeration types in SPARK.

\begin{minted}{ada}
type Color is (Red,Green,Blue);
\end{minted}

4. Arrays: (\texttt{|type\\s+[a-zA-Z\_]+\\s+is\\s+array\\s*(\\d+)|}. Matches array declarations.

\begin{minted}{ada}
type Int_Array is array (1 .. 10) of Integer;
\end{minted}

5. Dynamic Vectors: (Combines \verb|range <>|, \verb|Containers.Vector| and \verb|Containers|).
...
6. References: (Combines: \verb|type\s+\w+\s+is\s+access\s+\w+| and \verb|new\s+\w+)|). This pattern matches access types (pointers) and memory allocations using 'new'.

\begin{minted}{ada}
type Int_Ptr is access Integer;
\end{minted}

7. Unbounded Strings: (\verb|Strings.Unbounded|). This matches unbounded strings from the Ada.Strings.Unbounded package.
...
\begin{figure}[ht!]
  ...
  \scalebox{0.9}{
    ...
    \foreach \p/\t/\pat\c in {56.06/Numbers/crosshatch/red!70!white, 24.71/Refs/vertical lines/orange!90!white, 11.18/Struct\&Enum/grid/yellow!90!orange, 8.05/others (heap)/horizontal lines/green!60!black}
       {
        \global\lena=\lenb
