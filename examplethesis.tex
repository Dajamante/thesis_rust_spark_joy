...
\begin{figure}[!ht]
  ...
  \end{center}
  \caption[Software complexity in the automotive and aerospace industry]{Software complexity in the automotive and aerospace industry from\,\cite{moy_when_2020}}
...
\begin{figure}[ht!]
    ...
    % group 2
         \node[punktchain, join, draw=black,fill=yellow!50!white
         ] (typselec){Type selection};
         \node[punktchain, join, draw=black,fill=yellow!50!white
         ] (stats){Statistics};
    % group 3
         \node[punktchain, join, draw=black,fill=brown!50!white
         ] (cs){Control studies};
         \node[punktchain, join, draw=black,fill=brown!50!white
         ] (analys){Analysis};   
         \node[punktchain, join, draw=black,fill=brown!50!white
...
Rust compiler is quite liberal, and the type does not need to be declared as long as the compiler can figure out the type from the number. For this reason, we looked at number literals. 

The number regex: \verb|\b(?:\d+(?:\.\d\+)?\d+(?:\_\d\+)\+)\b| matches integers and floats. It matches also numbers with an underscore, as authorized by Rust. Because it is fenced by boundaries, it should not match type declarations or numbers inside a struct (please see next regex).
...
\texttt{struct} regex and \texttt{enum} regex. Since structs and enums need to be explicitly declared.
...
\texttt{array} regex: This regex matches array literals \inlinecode{[1, 2, 3, 4, 5]}, but not their declaration \inlinecode{[i32;5]}. More often then not the compiler does not need the declaration and can figure out the type. But in case we have the declaration, we don't want to match twice.

\texttt{references} regex: This regex matches reference types in Rust, including mutable and immutable references.
...
\texttt{heap} regex: This regex matches the declaration heap-allocated types, specifically Box, Rc, and Arc. The regex is the keyword followed by \texttt{<}.

\begin{minted}{rust}
let boxed_value: Box<i32> = Box::new(5);
let rc_value: Rc<String> = Rc::new(String::from("Hello"));
let arc_value: Arc<u8>;
\end{minted}

\texttt{vector} regex: This regex matches dynamic Vec types and the vec![] macro in Rust.
...
\texttt{string} regex pattern matches instances where a String is created. The usual methods are \inlinecode{to\_string()}, \inlinecode{String::new()},  \inlinecode{String::from()} or \inlinecode{format!()}.
...
\label{tab:crates_io}
\begin{tabular}{ |l|>{\centering\arraybackslash}m{3cm}|>{\centering\arraybackslash}m{3cm}|>{\centering\arraybackslash}m{2cm}| }
\hline
\rowcolor{orange!50}
...
\footnotesize
\centering
\caption{Most popular projects on lib.rs 172,049 LOC} 
\label{tab:lib_rs}
\begin{tabular}{ |l|>{\centering\arraybackslash}m{3cm}|>{\centering\arraybackslash}m{3cm}|>{\centering\arraybackslash}m{2cm}| }
\hline
\rowcolor{orange!50}
...
\hline
serde & Framework for serializing and deserializing & b80e722 (24,754) & 151 (30,681)  \\
\hline
serde-json &  A JSON serialization file format  & a15bd09 (16,488) & 126  (15,553) \\
\hline
clap & Command line args parser & 8469554 (10,201) & 106 (49,799) \\ 
\hline
log &  A lightweight logging facade for Rust  & dc32ab9 (12,158) &  135 (3,322) \\ 
\hline
thiserror &  Error handler  & 0e45dde (9,010) &   83 (2,812) \\ 
\hline
tokio &  Non-blocking platform for asynchronous I/O backed applications & 1df874e (11,583) & 89 (69,892) \\
\hline
\end{tabular}
\end{table}
\FloatBarrier
...
\pgfplotstableread{
Rust_type lib_rs crates_io rustc
Numbers  22430 44688 304618
Refs 7140 13909 137530
Arrays  409 1893 9291
Vec  1302 397 14278
Enum  1283 602 15040
Struct  3646 4704 48955
String  1363 445 18613
}\EXPDATA 
...
\begin{figure}
    ...
    \begin{tikzpicture}
        ...
        \legend{crates.io (171 kLOC),lib.rs (172 kLOC),rustc (1518 kLOC)}
        \end{axis}  
    \end{tikzpicture}}
...
cratesio   68.17   19.32  8.10  1.53  2.88
librs   49.95   30.97  10.98  7.20  0.90
rustc   54.86   24.76  11.53 7.18 1.67
}\testdata
...
%                },
            xshift={  50pt },
                yshift ={
                ifthenelse((\plotnum == 4), 30pt,25pt)},
            },
...
\begin{figure}[ht!]
  ...
  \scalebox{0.9}{
    ...
    \foreach \p/\t/\pat\c in {55.84/Numbers/crosshatch/red!70!white, 24.65/Refs/vertical lines/orange!90!white, 11.30/Struct\&Enum/grid/yellow!90!orange, 8.21/others (heap)/horizontal lines/green!60!black}
      {
        \global\lena=\lenb
...
\begin{figure}[ht!]
  ...
  \scalebox{0.9}{
    \begin{tikzpicture}[scale=3]
    \centering    
    \foreach \p/\t\pat\c in {51.84/Numbers/crosshatch/red!70!white, 32.15/Access/vertical lines/orange!90!white, 14.42/Record\&Enum/grid/yellow!90!orange, 1.59/others (1.59\%)/horizontal lines/green!60!black}   
      {
...
\end{itemize}
\subsection{Threat to validity in the statistics}

We do not foresee large threats to validity for Rust. Rust is a systems programming languages and appears to be used a such. The most significant threat to the validity of the statistics concerns the experiments on SPARK. Large SPARK code bases are proprietary, forcing us to operate statistical analysis on small \gls{OSS} projects. As a counterpoint, we can rely here on expert opinion on how SPARK code is used in real projects (Moy, in direct conversation). Besides, SPARK has a long industrial track record. It is used in many industrial applications, including air traffic, avionics, railways \etc. It is based on Ada, which is general-purpose and based on the classical stack model. Being used in industrial applications, one can assume safely that effectivity is a concern to SPARK programmers\,\cite{chapman_adacore_2018}.
...
\subsection{Additional tools for debugging and tracking memory usage}
...
\todoinline{Link to the different GitHub repos with a description of what they do including stats, do the README to be more friendly}
...
We experimented with SPARK to/from Rust systems. Both setups had similarities but also differences. 

For reproducibility purposes, this section describes the setup from a high-level perspective while the source code is available in \Cref{sec:systemDocumentation}.


\subsection{SPARK to Rust}

\Cref{fig:sparkbuild} shows the build process for a SPARK file using a Rust library. Both files reside in the same root folder and this is a straightforward process. 
...
The process is designed to fail if a step is not done properly.
...
This time we are using the crate \texttt{gpr} which is designed for bigger projects. While it introduces a more complexity, it also allows access to a richer toolset.  
...
\section{Experiments stack-types}

Stack-types sizes are known at compile time in both Rust and SPARK. For numbers, Rust types names clarify its size, while the GNAT compiler chooses the best type to hold the variable depending on the precision that is required (**learned that in the training yesterday, to develop?).

A series of experiments were done on stack-allocated scalar and aggregate types. As a reminder, a scalar type is a type that can hold a single value at a time (for example, an integer or an enum). Conversely, a non-scalar type holds different values, like an array. 

The data explored in the experiments include arithmetic manipulations, ensuring correct layout and size, and shows that compatibility is relatively easily achieved, even if human error is still possible. The goal of our experiments is not to write defensive code which could never be exhaustive. That being said, loosing contract based programming in SPARK and Rust compiler verification showed how brittle the FFI passing could be (** I mean here that I read that in the literature but I could see properly what was meant).

We tested basic integer types, with simple integer arithmetic and swapping integer values. They were passed by copy and reference, and the result was returned or updated in place.

We tested fieldless enums to shows that the memory layout is compatible between the two languages by using the repr attribute in Rust and the Size aspect in Ada, since fieldless enums are internally resolved to tags.

We tested arrays of integers and fixed-size strings. We highlighted possible unsafe operations, that could be prevented by sanitizing.

Those experiments demonstrated how to perform sound and safe operations with SPARK/Rust code in simple operations with stack-allocated data.

\section{Experiments on heap types}

For heap-types, the focus was laid on memory management. 

The heap experiments focus on arithmetic operations and writing at specific locations, for example replacing the bytes inside a String.

The experiment is done both with a String and records containing heterogenous data types (access String and Integer).
This part focuses on meeting layout expectations. We looked at the size and layout of data structures we were passing (with flags and \inlinecode{std::mem}). We experimented with padding and disassembled the program. We compared addresses to make sure that the same object was fetched on both sides.
We saw that the expectations of SPARK must be respected. For example, fat pointers must be reproduced with the same layout, with a similar data structure on the other side.
Also, memory management needs to be done in a good FFI citizen way. data allocated on one side must be freed on the same side.

While still not focusing on defensive code, we exported all the unsafe operations to a part of the code where safety checks were done, allowing proper error handling.

We also control for not unwinding at the FFI frontier, which is undefined behavior.

We run \texttt{valgrind} to show both sound bindings and bindings which have intentional memory leaks, as a demonstration.

\section{BBqueue}

\todoinline{BBqueue experiments}
...
\todoinline{This discussion on numbers will go to the discussion together with the recommendation}

\todoinline{This is what we found out about numbers, and with its recommendation will be moved to the discussion part.}
