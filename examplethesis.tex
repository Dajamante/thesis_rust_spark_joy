...
while the github links to the code is available in \Cref{sec:systemDocumentation}. 

\section{List and description of experiments}
\label{tab:listexperiments}

\Cref{tab:fromsparktorust} and \Cref{tab:fromrusttospark} list the control studies and the behaviour being tested. The experiment in BBQueue gathers the lessons learned from control studies.\Cref{sec:experimentstack} describe our focus with types allocated on the stack, and \Cref{sec:experimentsheap} describe our focus for dynamically allocated types.
...
\todoinline{type safety: for the stack types, the issue encountered could be mostly solved by finding the right types, and verifying the alignment was correct.  the issues can be solved by selection of the right type through the documentation or creation of the right type for nested types. memory type: no issue, ownership: no issue \gls{copy-type} types are taken care of by the compiler}
\subsection{Numbers}
\label{sec:numbers}
...
\todoinline{type safety:  the issue encountered could be mostly solved by finding the right types, and verifying the alignment was correct. memory safety: while both compiler provide their own memory safety mechanism, we could not rely on those. It was important to firstly go back to working with pointers and separating pointer logic from language logic, and secondly to limit the unsafe parts to the minimum possible: ownership: we rely on the language as much as possible, we default to the pointer and good ffi citizen for ownership}
...
\todoinline{learn rules for unsafe behaviour: unsafe behaviour has clear rules (refer to last book) that needs to be respected even more draconianly when working with FFI }

\todoinline{learn undefined behavior for each language- do not rely on sanitizers - an automated tool should absolutely check the size of the array}
