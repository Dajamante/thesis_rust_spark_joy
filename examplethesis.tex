...
4. Arrays: (\verb|type\s+[a-zA-Z\_]+\s+is\s+array\s*(\d+)|). Matches array declarations.
...
\begin{figure}[ht!]
  ...
  \scalebox{0.9}{
    \begin{tikzpicture}[scale=3]
    \centering    
    \foreach \p/\t\pat\c in {58.69/Numbers/crosshatch/red!70!white, 27.78/Access/vertical lines/orange!90!white, 12.65/Record\&Enum/grid/yellow!90!orange, 0.88/others (0.88\%)/horizontal lines/green!60!black}   
      {
...
shows the build process for a SPARK file using a Rust library. 
...
Stack-types sizes are known at compile time in both Rust and SPARK. For numbers, Rust types are explicit regarding their size, while the GNAT compiler chooses the best type to hold the variable depending on the precision that is required.
