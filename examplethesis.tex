...
\begin{figure}[ht!]
    \centering 
      \includesvg[inkscapelatex=false, scale=0.8]{figures/figuressvg/bbqueueprocess.svg}
      \caption{BBqueue process}
\end{figure}
\FloatBarrier
...
\begin{figure}[ht!]
  \centering \includesvg[inkscapelatex=false, scale=0.8]{figures/figuressvg/bbqueuecom.svg}
  \caption{Data flow in BBqueue}
\end{figure}

\subsection{Implementation details}

In Rust, BBqueue has two implementations. The official one is not \glsxtrshort{FFI} safe, and we will be using the version developed for \href{https://github.com/tosc-rs/mnemos/tree/main/source/abi/src/bbqueue_ipc}{mnemos OS} for this experiment.  
...
We experimented with SPARK to/from Rust systems. For reproducibility purposes,  \Cref{sec:buildSystems} describes the setup from a high-level perspective while the source code is available in \Cref{sec:systemDocumentation}.
...
The objective of this thesis is to procure a series of recommendations ("\textit{Do's}") that apply to an automated tool for bindings on the model of \texttt{bindgen}\,\cite{noauthor_bindgen_2022}. Correctness is evaluated if there is no memory error with \texttt{valgrind} -this does not prove the correctness but the absence of memory error.
...
4. While SPARK verifications (pragma and contract based) are lost when interfacing with Rust, we can still implement thick bindings.
...
This is the first type where layout plays a big role.


\subsection{Heap types and pointers}

The main point about pointers in Ada/SPARK and Rust is that they carry more information than C, and we thought about how to carry this information to the language accross the FFI barrier.
In this part we worked on pointers. Pointers contains a lot of information in both Ada/SPARK and Rust. 
In SPARK, they have nominal equality, instead of structural equality (the access to type T1 is access Integer will not be equal to type T2 is access Integer). 
In Ada/SPARK pointers cannot escape as they are protected by types. 

In Rust, the \&mut type carries several assumptions. It is never null, it is pointing to valid data and the Rust program can be certain that it is never null. 

We worked mainly with the String type. The String type in both Rust and SPARK is a complex type. The challenge was how to represent on the other side all the information carried by the fat pointers, and how to rely on the language semantics to have all the security operations "for free".

Rust strings are UTF-8 encoded, and even their length is not obvious depending if one is counting bytes, characters or graphemes. Consider:
...
\begin{figure}[ht!]
    ...
    tuborg/.style={decorate, decoration={brace, amplitude=3pt}, line width=1mm},
    tubnode/.style={midway, right=2pt, align=left, font=\large, text width=4cm, xshift=4mm},
    ]         \node[punktchain, join, draw=black,fill=red!40!white] (string){Vec<u8> of UTF-8};  
    % group 2
    \node[punktchain, join, draw=black,fill=yellow!50!white] (vec){Vec<T>};
    \node[punktchain, join, draw=black,fill=brown!50!white] (rawvec){RawVec<T>};
    \end{tikzpicture}}
  \caption{Rust String}
...
\begin{figure}[ht!]
    ...
    \scalebox{0.8}{
    \begin{tikzpicture}
	\node [draw, rectangle, fill=blue!20, minimum width=1cm, minimum height=1cm] (box1) at (0,0) {first};
	\node [draw, rectangle, fill=blue!20, minimum width=1cm, minimum height=1cm, right=0cm of box1] (box2) {last};
	\node [draw, rectangle, fill=yellow!20, minimum width=1cm, minimum height=1cm, right=0cm of box2] (box3) {'h'};
	\node [draw, rectangle, fill=yellow!20, minimum width=1cm, minimum height=1cm, right=0cm of box3] (box4) {'e'};
	\node [draw, rectangle, fill=yellow!20, minimum width=1cm, minimum height=1cm, right=0cm of box4] (box5) {'l'};
	\node [draw, rectangle, fill=yellow!20, minimum width=1cm, minimum height=1cm, right=0cm of box5] (box6) {'l'};
	\node [draw, rectangle, fill=yellow!20, minimum width=1cm, minimum height=1cm, right=0cm of box6] (box7) {'o'};
  ...
  \caption{Ada/SPARK unconstrained String}
...
\begin{figure}[ht!]
    \centering \includesvg[inkscapelatex=false, scale=0.7]{figures/figuressvg/stringlogic.svg}
      \caption{Business logic separation}
...
As per previous considerations of being a good FFI citizen in \Cref{fig:businesslogic}, the Rust side must reconstruct the language and implement the drop behaviour.
...
The headers were different enough
\cleardoublepage
