...
This thesis aims to explore the challenges of combining memory-safe programming languages Rust and SPARK.

SPARK is a subset of Ada, a programming language with a long-standing track record in the safety-critical industry. Rust is a multi-paradigm language that has gained significant popularity since its emergence roughly a decade ago. Their ownership system and type-safety help in the development of safe and reliable software.

Memory-safety bugs in code written in C/C++ account for about \SI{70}{\percent} of major vulnerabilities in the industry, as reported by organizations such as Microsoft, macOS, and Google. While tools to avoid memory errors exist, code can not be made safe retroactively, which creates a need for languages such as Rust and SPARK which were designed with safety in mind. 

Combining those two languages is a very promising path for low-level and systems programming, as well as safety-critical applications (automotive, avionics, medical\ldots). This thesis aims to identify best practices for safe bindings between Rust and SPARK, focusing on ensuring memory safety, type safety, and ownership, and analyzing how they are maintained or transferred. The thesis provides clear recommendations based on control study programs and a real-world example using BBQueue, a circular buffer library suitable for embedded systems, which is implemented in both languages. The identified best practices can be used for safe bindings and for automated tools/code generation. 

This thesis fills a gap in existing research, which has largely focused on the combination of safe and unsafe languages and provides a valuable contribution to the field of software security and safety-critical systems.
...
Moy and Aiello\,\cite{moy_when_2020} say that as products integrate and start to rely on software for their functionality and move from hardware-only to the cyber-physical space, they become vulnerable, which rings especially true for the \gls{safety-critical} industry. Furthermore, they warn that software complexity is increasing (see \Cref{fig:complexity}), aggravating challenges to software security.
...
\begin{figure}[!ht]
  ...
  \end{center}
  \caption{Software complexity in the automotive and aerospace industry from\,\cite{moy_when_2020}}
  \label{fig:complexity}
\end{figure}


Moy and Aiello\,\cite{moy_when_2020} report security incidents which brought down AT\&T (at a cost of US\$60 million and long-lasting damage to the company's reputation), the \gls{FDA} warned against almost 500 thousand cardiac devices that put the patients in pain or at risk of death, and the infamous WannaCry ransomware attack that encrypted 200 thousand computers in 150 countries with a reported cost of US\$8 billion.
...
\subsection{Can we reach memory safety by using safe languages?}
...
~\Cref{ch:relatedworks} discuss related works when it comes to \gls{FFI}.  \Cref{ch:methods} presents the methodology and method used to solve the problem. …
...
\begin{sidewaysfigure}[!ht]
  ...
  \end{center}
  \caption{Attack model demonstrating four exploit types and policies mitigating the attacks in different stages by Szekeres \etal\,\cite{szekeres_sok_2013}  \textcopyright2013 IEEE}
  \label{fig:sok}
\end{sidewaysfigure}
...
\subsection{A new cross-language paradigm for the programmer}
...
\cleardoublepage
\chapter{Method}
\todoinline{Choose possible title later, for example, "Method: Experiments and Real-world implementation"}
...
This chapter provides an overview of the research and experimentation method. Section~\ref{sec:researchProcess} describes the research process. Section~\ref{sec:researchParadigm} details the research paradigm. 
...
describes the experimental design. Finally, Section~\ref{sec:systemDocumentation}
describes the system to ensure reproducibility.
\todoinline{I will need to add a section to detail the experiments on BBQueue later when it comes to that.}
...
\begin{figure}[ht!]
    ...
    % group 1
         \node[punktchain, join, draw=black, fill=red!40!white
         %fill=black!15
         ] (lit){Litterature study};
         \node[punktchain, join, draw=black, 
         fill=red!40!white
         %fill=black!15
         ] (consult){Consultation with experts};
         \node[punktchain, join, draw=black, 
         %fill=black!15
         fill=red!40!white
    ...
    % group 3
         ...
         %fill=black!35
         fill=brown!50!white
         ] (memver){Verification of correctness};   
    % group 4
         \node[punktchain, join, draw=black, 
         %fill=black!45
         fill=orange
         ] (bbq){BBQueue implementation};
    %% No. 1
...
The process is constituted of four steps:
...
The data types selected are series of stack and heap objects. 

The choice can be justified with a statistical tool that shows which types are widely used in Rust and SPARK projects (\cref{sec:stats}).

\subsection{Statistic tool}
\label{sec:stats}
We created a program analyzing 17 code bases in Rust, including the Rust compiler, and three open-source code bases in SPARK. 
%\inlinecode{x=2}


\todoinline{make a friendly README for the stat repo and link it, link it also at the end}

\todoinline{new research question: how can we select useful types? 
possible reply: it seems to be scalar types and pointers are widely used in both languages}

The program counts occurrences of different patterns with Regex in the code bases, including numbers, arrays, references, strings, and various heap-allocated types. 

\texttt{cloc 1.9} counted 26,137 Rust files (1,792 k\gls{LOC}) in popular and qualitative Rust repositories.
...
The data is collected via file parsing and \texttt{Regex} patterns. 

One source of error can be that some data types can be in comments instead of code. This worry is legitimate but can be put aside given the gap between the used data types, where numbers clearly dominate. 

When it comes to Rust mutable and immutable references, and admittedly references can point at any data structure. But in the experiments references are used to study how ownership is transmitted, rather than a data type in its own rights. The presence of many \inlinecode{\&mut T} and \inlinecode{\&T}, as well as \inlinecode{Access} types in SPARK, shows that we need to study references and pointers.

The most significant threat to the validity of the statistics is that large SPARK code bases are proprietary, forcing us to operate statistical analysis on small OSS projects. As a counterpoint, we can rely here on expert opinion on how SPARK code is used in real projects (Moy, in direct conversation). Besides, SPARK has a long industrial track record. It is used in many industrial
applications, including air traffic, avionics, railways \etc. It is based on Ada, which is general-purpose and based on the classical stack model. Being used in industrial applications, one can assume safely that effectivity is a concern to SPARK programmers\,\cite{chapman_adacore_2018}.
\subsection{Rust}

\todoinline{Describe all the tools considered and used except valgrind and rust san. ffichecker, then the auri thing, then rr, miri}

Rust is an open-source project with open crates (libraries). To perform the statistics below, the most popular crates were selected on Rust's official pages for crates, \href{crates.io}{crates.io}. Those crates are mostly used in the compiler, and for this reason, we selected popular projects on \href{lib.rs}{lib.rs} to analyze what programmers are actively using in production\footnote{Lib.rs utilizes a ranking algorithm that offers more qualitative information than crates.io, filtering away spam and poor-quality projects. The crate popularity is measured by downloads, direct and reverse dependencies, the usage trend, quality of README, tests, comments documentation, examples, and whether the crate is actively maintained}.
Thirdly, the compiler, with a code base over 5 times bigger than the two projects above, was also deemed relevant for statistics.

\todoinline{REPLACE VERSIONS WITH HASH 
librs/log: dc32ab9
librs/json: a15bd09
librs/thiserror: 0e45dde
librs/clap: 8469554
librs/serde: b80e722
rustc: 66a2d62
cratesio/syn: 6365093
cratesio/rand: 0f3eced
cratesio/libc: 89ec881
cratesio/quote: ca98b65
cratesio/cfg-if: 2621a59
cratesio/proc-macro2: ab25487
ewok-kernel: ca8e2a0
SPARKNaCl: dfb1bd1
spark-by-example: 2e4eb5a
}
...
\caption{crates.io}
\label{tab:crates_io}
\begin{tabular}{ |l|m{3cm}|>{\centering\arraybackslash}m{3cm}|>{\centering\arraybackslash}m{2cm}| }
\hline
\rowcolor{brown!50!white}
\multicolumn{4}{|c|}{Most popular crates.io projects 171 kLOC} \\
\hline
\hline
\rowcolor{orange!50}
Package& Description & Version \newline (Reverse Dependencies)&Downloads \newline  all time (in millions)\\
\hline
\footnotesize
libc & Raw FFI bindings to platform libraries like libc & v0.2 (5,200) & 177 \\
\hline
syn & Parser for Rust source code & v1.0 (5,501) & 191 \\
\hline
rand and rand-core & Random number generators and other randomness functionality & v0.8 (9,892) & 183 \\
\hline
quote & Macro for turning Rust syntax tree data structures into tokens of source code & v1.0 (5,498) & 177 \\
\hline
cfg\-if & Ergonomically defines an item depending on a large number of \#[cfg]  & v1.0 (1,147) & 175 \\
\hline
procmacro2 & A substitute implementation of the compiler's proc\_macro API & v1.0 (4,538) & 176 \\
\hline
\end{tabular}
\end{table}
\begin{table}[ht!]
\centering
\caption{Most popular projects on lib.rs 102 kLOC}
\label{tab:lib}
\begin{tabular}{ |l|m{3cm}|>{\centering\arraybackslash}m{3cm}|>{\centering\arraybackslash}m{2cm}| }
\hline
\rowcolor{brown!50!white}
\multicolumn{4}{|c|}{lib.rs} \\
\hline
\hline
\rowcolor{orange!50}
Package& Description & Version \newline (Reverse Dependencies)&Downloads \newline  all time (in millions)\\
\hline
    serde & Framework for serializing and deserializing & v1.0 (24,754) & 151  \\
    \hline
    serde-json &  A JSON serialization file format  & v1.0 (16,488) & 126   \\
    \hline
    clap & Command line args parser & v4.1 (10,201) & 106  \\ 
    \hline
    log &  A lightweight logging facade for Rust  & v0.4 (12,158) &  135  \\ 
    \hline
    thiserror &  Error handler  & v1.0 (9,010) &   83  \\ 
    \hline
    tokio &  Non-blocking platform for asynchronous I/O backed applications & v1.26 (11,583) & 89 \\
\hline
\end{tabular}
\end{table}
\FloatBarrier
...
When looking at types per project, we can see that the rust compiler is much bigger \cref{fig:barplottypesrust}, for comparison, we have 102 k\gls{LOC} for lib.rs,  171 kLOC for crates.io and 1,518 kLOC in the rust compiler, but the repartition is quite similar with a domination of numbers \cref{fig:barplottypesrustpercent}. 

But as we can see, numbers (Integers and Float) dominate the statistics \cref{fig:repartitionrust}. 

Their size is known at compile time, they are expressive and protected against overflow\footnote{The compiler will warn against risks of overflow}. 

This confirms that Rust is used as a low-level, systems programming language. 

Coming second are \inlinecode{\&mut T} and \inlinecode{\&T}. References are a key element of Rust's ownership system and are used in a fourth of the cases. Logically, heap types account for a minority of types.

\pgfplotstableread{
Rust_type rustc crates_io lib_rs
Numbers  12680 44688 304618
Refs 7140 12662 137530
Arrays  130 1893 9291
Vec  977 397 14278
Enum  1162 602 15040
Struct  2428 4704 48955
String  1140 445 18613
}\EXPDATA 
...
\begin{figure}
    ...
    \begin{tikzpicture}
        \begin{axis}[
            %title={Rust data types per project},
            ...
            ymode=log,
            width=1\textwidth,
            ylabel=Absolut number of types,
            ]
        ...
        \legend{crates.io (171 kLOC),lib.rs (102 kLOC),rustc (1518 kLOC)}
        \end{axis}  
    \end{tikzpicture}
...
Numbers 68.17 49.17 54.86 
Refs 19.32 27.69 24.77 
Struct 7.18 9.42 8.82
Enum 0.92 4.51 2.71
Vec 0.61 3.79 2.57
String 0.68 4.42 3.35
Box 0.24 0.51 1.26
Arrays 2.89 0.50 1.67
}\PERCENTDATA
...
\begin{figure}[ht!]
    ...
    \begin{tikzpicture}
        \begin{axis}[
            cycle list name=rustcolors,
            xtick=data,
            symbolic x coords={Numbers,Refs,Struct,Enum,Vec,String,Box,Arrays},
            ybar,
            legend style={at={(0.5,-0.2)},anchor=north},
            ymajorgrids=true,
            grid style=dashed,
          ...
          \addplot table [x=Rust_type, y=rustc]{\PERCENTDATA} ;
          \addplot table [x=Rust_type, y=crates_io]{\PERCENTDATA} ;
          \addplot table [x=Rust_type, y=lib_rs]{\PERCENTDATA} ;
        \legend{crates.io, lib.rs,rustc}
        \end{axis}  
    \end{tikzpicture}
\caption{Rust types repartition for the compilers, crates.io and lib.rs (percentage)}
\label{fig:barplottypesrustpercent}
\end{figure}

\begin{comment}
Detailed table
Label Numbers  Refs  Struct  Enum  Vec  String  Box  Arrays
cratesio   68.17   19.32  7.18    0.92  0.61  0.68   0.24  2.89
librs   49.17   27.69  9.42    4.51  3.79  4.42   0.51  0.50
rustc   54.86   24.77  8.82    2.71  2.57  3.35   1.26  1.67
\end{comment}
\pgfplotstableread{
Label Numbers  Refs  Struct/Enum  Heap  Arrays
cratesio   68.17   19.32  8.1  1.53  2.89
librs   49.17   27.69  13.91  8.72  0.50
rustc   54.86   24.77  11.53 7.18 1.67
}\testdata
...
\begin{figure}[ht!]
    ...
    \end{axis}
    \end{tikzpicture}
\caption{BAR STACKED TEST (With percentage)}
\label{fig:stacked}
\end{figure}
...
\foreach \p/\t/\c in {56.0/Numbers/red!40!white, 24.3/ Refs/yellow!50!white, 11.3/Struct\&Enum/brown!50!white, 8.38/others (heap)/orange}
  {
    \global\lena=\lenb
...
%\begin{table}[ht!]
%  \centering
%  \caption{Rust Types Data}
%    \begin{tabular}{|l|r|r|}
%   \hline
%    Rust Type & Frequency of Use & Percentage \\
%    \hline
%    Numbers & 362,648 & 56.20\% \\
%    \&mut T or \&T & 155,911 & 24.16\% \\
%    struct & 55,716 & 8.68\% \\
%    enum & 16,677 & 2.58\% \\
%    Vec & 15,613 & 2.46\% \\
%    Array & 11,418 & 1.77\% \\
%    String & 20,012 & 3.10\% \\
%    Box/Arc/Rc & 7,217 & 1.12\% \\
%    \hline
%    Total & 654,124 & 100.00\% \\
%    \hline
%    \end{tabular}%
%  \label{tab:rust_types}%
%\end{table}%
\FloatBarrier
...
\begin{itemize}
    \item \textbf{SPARKNaCl}, a cryptography library that implements the same functionality as the NaCl (a networking crypto library), which aims to provide a completely automated static proof of type-safety.
    \item \textbf{EwoK}, a secure microkernel designed for micro-controllers and embedded systems that enforces strict isolation between tasks and device drivers, and provides strong access control to physical resources. 
    \item \textbf{SPARK by example}, a repository with exercises to practice the language
\end{itemize}
...
%Total number of types : 2695
%key : Record, value : 310, percentage 0.1150
%key : Number, value : 1397, percentage 0.5184
%key : Enum, value : 86, percentage 0.0319
%key : Unconstrained array, value : 28, percentage 0.0104
%key : Access, value : 873, percentage 0.3239
%key : Unbounded strings, value : 1, percentage 0.0004

\foreach \p/\t\c in {51.8/Numbers/red!40!white, 32.4/Access/yellow!50!white, 14.6/Record\&Enum/brown!50!white, 1.6/others(1.6\%)/orange}
  {
...
The control studies will be centered around:
\begin{itemize}
    \item Stack objects and scalars: numbers, arrays, enums, and structs (with objects size known at compile time).
    \item Heap objects: dynamic types such as Vec.
\end{itemize}

The focus will be on \texttt{\&mut T} and \texttt{\&T}, as well as the \texttt{Access} types, to keep or transmit ownership.

\todoinline{Describe more the BBQueue test when we come to that}

\subsection{Environment}

\todoinline{Describe briefly the Dockerfile and link to the repo}

\subsection{Hardware/Software to be used}

\todoinline{Describe my settings}


\subsection{Validity of method}
\label{sec:validtyOfMethod}

The objective of this thesis is to procure a series of recommendations ("\textit{Do's}") that are applicable to an automated tool for bindings on the model of \texttt{bindgen}\,\cite{noauthor_bindgen_2022}. 

As a reminder, bindgen is a binding tool that generates bindings between Rust and C. The languages analyzed in this thesis have richer types than C, and the goal is to learn to take full advantage of their expressivity.

The method chosen is manually doing those bindings through control studies and analyzing them with help of the documentation for each type as well as memory tools to ensure the binding is correct and that there is no memory leak. It is necessary to establish that ownership, type safety, and memory safety are conserved for a binding to be considered correct.
...
When the control studies are performed, an additional experiment is executed with a library that exists in both languages.
...
Linking is always a major problem in this kind of project.

From SPARK to rust I used direct linking with memory sanitizer at the gcc level or rustc level.
From Rust to SPARK I was lucky enough to use a more advanced tool that has its own build script.

\todoinline{Describe the above}


\todoinline{Add valgring and flags}
...
\todoinline{Link to the different github repos with description of what they do including stats, do the README to be more friendly}
...
%control study
\cleardoublepage
\chapter{Experiments and elements of discussion}
\label{ch:whatYouDid}


\todoinline{Currently only experiments, need to add BBqueue when we come to that.}
...
This section covers the experiments done to implement our FFI bindings between Rust and SPARK. 

For reproducibility purposes, it will describe the setup from a high level perspective.
Efforts were put into going away from in between C types and going away from C as a protocol.

After assessing for type safety and ownership, we ran tools for memory safety.

FFI-checker, rr and memflow were not used as valgrind was more than sufficient (FFI-checker had dependencies issues, rr had issues on AMD and memflow was an overkill).
We used graphs to visualise control flow to see which types were copy (graphviz).

We are using gnatprove whenever we can.

\section{System}
Linux version 5.19.0-051900-generic (kernel@sita) (x86\_64-linux-gnu-gcc-11 (Ubuntu 11.3.0-5ubuntu1) 11.3.0, GNU ld (GNU Binutils for Ubuntu) 2.38.90.20220713) \#202207312230 SMP PREEMPT\_DYNAMIC Sun Jul 31 22:34:11 UTC 2022

% the following sets the TOC entry to break after the & - note you have to include the first letter of the following word as it get swolled by the \texorpdfstring{}{} processing
\section[Environment for \& SPARK/Rust experiments]{Environment for SPARK/Rust experiments}


Figure~\ref{fig:buildada} shows the build process for a SPARK file using a Rust library. Both files reside in the same directory. This is a straightforward process where the rust lib is compiled as a staticlib.
...
\begin{tikzpicture}[node distance=2cm, font=\sffamily]
    ...
    \draw [arrow] (step1) -- (step2) node[midway, right] {gcc, no optimisation};
    \draw [arrow] (step2) -- (step3) node[midway, right] {build the rust file as staticlib with rustc};
...
\begin{figure}[ht!]
    ...
    \node (step2) [process, fill=yellow!20, below of=step1] {Compiling Ada files};
    \node (step3) [process, fill=brown!20, below of=step2] {Building};
    \draw [arrow] (step1) -- (step2) node[midway, right] {Build with call to gprbuild};
    \draw [arrow] (step2) -- (step3) node[midway, right] {Linking};
...
\section{Experiments on types known at compile time}

\subsection{Description}

A series of experiments were done on scalar and aggregate types. For a reminder, a scalar type is a type that can hold a single value at a time (for example, an integer or an enum). Conversely, a non-scalar type example would be an array.

In this section we did experiments on integers, enums and arrays.

The goal of the experiments is demonstrate the integration of Rust functions into Ada through the FFI (foreign function interface) by performing simple operations like swapping integers, summing an array of integers, and printing an enumeration. The experiments illustrate the ability to pass integers and arrays by reference and pass enumerations (that are resolved to a list of numbers) between the two languages.

\textbf{Make a table with all the programs and their description.}

\subsection{Discussion on numbers}

What are the strong and weak points of Rust and Ada and how do we work around them?

Rust has Sized types u64 that are not machine dependent.

Ada has in addition Modular type and Bounded types.


\textbf{List of assumptions and discussion:}

Assumptions of the Rust program for Ada
1. It is going to be an i32 integer --> Integer on the Ada side
2. We don't know the size of the Integer in Ada. `usize` and `isize` (that used to be called int, dependent on the machine) may be `Integer` in Ada? Needs to be clarified
3. We are not using `usize` and `isize` as ``The size of this primitive is how many bytes it takes to reference any location in memory." --> they are not meant for calculation but for indexing
4. In Rust we should use sized type for calculation, but semantically = Integer\_ada are machine-dependent numbers// Rust they are not machine dependent
5. We should always use specific type sizes. Are there Ada libraries that give you scoped sizes? Like `u64`?
6. Remember it was a huge problem before: for example C's is using machine-dependent types
That was a huge problem before, Ada has 32 bits integers! The legality rules say that the Ada Integer size is between Min and Max of what the machine supports\url{https://ada-lang.io/docs/arm/AA-3/AA-3.5/#legality-rules-1}: 

From that, we see that GNAT provides also some references\url{https://docs.adacore.com/live/wave/gnat_rm/html/gnat_rm/gnat_rm/implementation_defined_characteristics.html#implementation-defined-characteristics}

7. What is happening

It is working without issues.

8. What can happen:

It is possible to "lie" to the compiler. Overflow can happen. SPARK cannot check.

9. Recommendations:
I expect the GNAT compiler to be used
I expect that sized types to be used
For constrained Integers the following must be done: …
For modular Integers the following must be done: …

Put some words on fieldless enums.
Put some words on arrays. We did it with Strings and it seem to work.
Those types are copy so there is no need to worry about it.
Copy types can be observed with MIR.


\section{Experiments on heap types}

In those experiments, we could focus on memory management.

Those still focus on arithmetic operations and writing at a specific location (bounded and unbounded string).
Working with heap-allocated data structures like strings and records containing accesses.
Memory management and deallocation were demonstrated.

The work focuses on meeting Ada expectations about the layout. Here we modeled the size of the datastructure with the flag -gnatRj, and did the same size on the rust side. we did some padding for example but it was a bad idea.
the expectations must be respected, for example ada fat pointers are first the data then the bounds, a similar data structure needs to be done on the rust side.
Also memory management needs to be done in good ffi citizen way. data allocated on one side must be freed on the same side.

rust is using jemalloc something something what the heck is ada using.

Also we showed it is easy to make memory errors by returning non boxed elements.
...
The prototype does not address the third requirement, \ie a yearly unavailability of less than 3 minutes, this remains an open problem. \ldots\ 

\subsubsection{Cost analysis}
\generalExpl{Example of a missing component}
The current prototype works, but the performance from a cost perspective makes this an impractical solution. Future work must reduce the cost of this solution, to do so a cost analysis needs to first be done. \ldots\ 

\subsubsection{Security}
\generalExpl{Example of a missing component}
A future research effort is needed to address the security holes that results from using a self-signed certificate. Page filling text mass. Page filling text mass. \ldots\ 


\subsection{Next obvious things to be done}

In particular, the author of this thesis wishes to point out xxxxxx remains as a problem to be solved. Solving this problem is the next thing that should be done. \ldots\ 
...
% \begin{itemize}[noitemsep]
%     \item \textcolor{kth-lightblue}{kth-lightblue \ifdigitaloutput actually Stratosphere
