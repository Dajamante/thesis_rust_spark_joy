...
\todoinline{describe the algorithm here}
\section{Environment}
...
\todoinline{add summary with florian and a graph of forbidden states}
...
\begin{figure}
    ...
    \begin{minipage}{.45\textwidth}
        \scalebox{0.8}{
            ...
            %\skippedwords \\
            %\wordbox[lrb]{1}{} 
            \bigmemsection{..AD0}{..B9F}{36}{Header (64B) - 8 fields of 8 bytes with atomic variables \tikzmark{end}}\\
    	    \bigmemsection{..B10}{..B9F}{16}{\tikzmark{end2} BBBuffer UnsafeCell<[u8; 128]> (128B+16B padding)}
            \end{bytefield}
        }
        %\caption{Header (64 bytes)and inner buffer (144 bytes)}
        \label{fig:hiddenbufferheader}
    \end{minipage}
        \begin{minipage}{.45\textwidth}
        \scalebox{0.8}{
            \begin{bytefield}{6}
            \tikzmark{start}\memsection{..BA0}{..BA7}{2}{0x..AD0: pointer to header}\\
    	    \tikzmark{start2}\memsection{..BA8}{..BAF}{2}{0x..B10: pointer to BBBuffer}\\
            \memsection{..BB0}{..BB7}{2}{length of read (4)}
            \end{bytefield}
        }
    \end{minipage}
    \begin{tikzpicture}[overlay,remember picture]
        \draw[->] (pic cs:start) -- %([xshift=-3.5cm, yshift=-4cm]pic cs:end);
        (pic cs:end);
        \draw[->] (pic cs:start2) -- (pic cs:end2);
    \end{tikzpicture}
